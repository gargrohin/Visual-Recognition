...
def get_full_clips(data_dir, num_clips, num_rec_out=1):
    ...
    ep_dirs = np.random.choice(glob(os.path.join(data_dir, '*')), num_clips,replace = False)
    ...
    for clip_num, ep_dir in enumerate(ep_dirs):
        ...
        start_index = np.random.choice(len(ep_frame_paths) - (c.HIST_LEN + num_rec_out - 1), replace = False)
